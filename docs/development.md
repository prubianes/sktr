...
SKTR requires Python 3.11 or newer. Install the locked development environment:
...
page and install `sktr` into a new Python 3.11+ environment.
...
For later releases, update `packages/sktr_core/version.py`, add the changelog
entry, rerun the full release checklist, and publish the matching version tag.
